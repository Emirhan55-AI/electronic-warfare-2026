...
| PHASE-06 | FPGA RTL DSP zinciri | Özel RTL dili SystemVerilog ve blok arayüzü AXI4-Stream olur. PHASE-06A vendor-bağımsız `ci8` giriş/frame temelini, PHASE-06B bit-doğru sabit nokta Hann'ı kurmuştur. PHASE-06C gerçek AMD IP'yi üretmeden 4096 FFT mimarisi, unscaled full-precision sayısal sözleşme ve vendor-independent wrapper/control sınırını doğrular. Sonraki kontrollü adımlar gerçek AMD IP entegrasyonu, lineer güç ve PHASE-03 `regional` detector RTL olur; sabit nokta, gecikme, kaynak ve RTL sonucu ilgili araçlarla ayrıca doğrulanır. Görsel profilin otomatik HDL ürettiği iddia edilmez. |
...
PHASE-06C sonrasındaki gerçek AMD FFT IP entegrasyonu, lineer güç ve PHASE-03 `regional` detector RTL adımlarının kesin faz kimlikleri sonraki planlama turlarında ayrıca kurulacaktır. PHASE-06C bu kimlikleri veya uygulanmamış işlevleri mevcut saymaz.
