...
| PHASE-06 | FPGA RTL DSP zinciri | Özel RTL dili SystemVerilog ve blok arayüzü AXI4-Stream olur. PHASE-06A vendor-bağımsız `ci8` giriş/frame temelini, PHASE-06B bit-doğru sabit nokta Hann'ı kurmuştur. PHASE-06C FFT mimarisi ve wrapper sınırını, PHASE-06D gerçek AMD FFT IP ile vendor doğrulamasını tamamlamıştır. PHASE-06E aynı gerçek wrapper/IP zincirinin Vivado synthesis, implementation, timing ve resource sonuçlarını doğrular; lineer güç ve detector ayrı sonraki alt-fazlardır. Görsel profilin otomatik HDL ürettiği iddia edilmez. |
...
PHASE-06E sonrasındaki lineer güç, PSD ve PHASE-03 `regional` detector RTL adımlarının kesin faz kimlikleri sonraki planlama turlarında ayrıca kurulacaktır. PHASE-06E bu işlevleri veya hardware çalışmasını mevcut saymaz.
