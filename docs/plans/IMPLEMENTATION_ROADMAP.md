...
| PHASE-06 | FPGA RTL DSP zinciri | Özel RTL dili SystemVerilog ve blok arayüzü AXI4-Stream olur. PHASE-06A vendor-bağımsız `ci8` giriş/frame temelini, PHASE-06B bit-doğru sabit nokta Hann'ı kurmuştur. PHASE-06C gerçek AMD IP'yi üretmeden 4096 FFT mimarisi, unscaled full-precision sayısal sözleşme ve vendor-independent wrapper/control sınırını doğrular. PHASE-06D gerçek AMD FFT IP üretimi, generated-port/config denetimi, AMD C-model sayısal karşılaştırması ve wrapper/core XSim entegrasyonunu kapsar; sentez, implementation, timing, kaynak, lineer güç ve detector bu alt-fazın dışındadır. Görsel profilin otomatik HDL ürettiği iddia edilmez. |
...
PHASE-05 kayıtlı/sentetik I/Q üzerinde operatör seçimli AM/NFM dinleme zincirini doğrulamıştır; bu sonuç PHASE-04 parametre doğrulamasının tamamlandığı anlamına gelmez. PHASE-06A/B/C tamamlanmış ve dondurulmuştur. PHASE-06D gerçek AMD FFT IP üretimi, C-model ve XSim işlevsel doğrulamasını tamamlamıştır; kapanış denetimi ve checkpoint işlemi ayrıca yapılır. Gerçek canlı HackRF dinleme, FFT sonrası power/detector zinciri, PHASE-07, PHASE-08 donanım kabulü ve TX başlatılmamıştır.
...
PHASE-06D sonrasındaki sentez/resource/timing, lineer güç, PSD ve PHASE-03 `regional` detector RTL adımlarının kesin faz kimlikleri sonraki planlama turlarında ayrıca kurulacaktır. PHASE-06D bu kimlikleri veya uygulanmamış işlevleri mevcut saymaz.
