...
| PHASE-06 | FPGA RTL DSP zinciri | Özel RTL dili SystemVerilog ve blok arayüzü AXI4-Stream olur. PHASE-06A vendor-bağımsız `ci8` giriş/frame temelini, PHASE-06B bit-doğru sabit nokta Hann'ı kurmuştur. PHASE-06C FFT mimarisi ve wrapper sınırını, PHASE-06D gerçek AMD FFT IP ile vendor doğrulamasını tamamlamıştır. PHASE-06E aynı gerçek wrapper/IP zincirinin Vivado synthesis, implementation, timing ve resource sonuçlarını doğrulamıştır. PHASE-06F exact FFT lineer güç RTL'sini kurar; PSD ve detector ayrı sonraki alt-fazlardır. Görsel profilin otomatik HDL ürettiği iddia edilmez. |
...
PHASE-05 kayıtlı/sentetik I/Q üzerinde operatör seçimli AM/NFM dinleme zincirini doğrulamıştır; bu sonuç PHASE-04 parametre doğrulamasının tamamlandığı anlamına gelmez. PHASE-06A/B/C/D tamamlanmış ve dondurulmuştur. PHASE-06E, gerçek AMD FFT wrapper/IP zincirinin Vivado synthesis, implementation, timing ve resource doğrulaması olarak sonuçlar görülmeden önce canonicalize edilmiştir. Gerçek canlı HackRF dinleme, FFT sonrası power/detector zinciri, PHASE-07, PHASE-08 donanım kabulü ve TX başlatılmamıştır.
...
PHASE-06F sonrasındaki PSD ve PHASE-03 `regional` detector RTL adımlarının kesin faz kimlikleri sonraki planlama turlarında ayrıca kurulacaktır. PHASE-06F bu işlevleri, post-power timing'i veya hardware çalışmasını mevcut saymaz.
