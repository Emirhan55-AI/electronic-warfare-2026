...
| PHASE-06 | FPGA RTL DSP zinciri | Özel RTL dili SystemVerilog ve blok arayüzü AXI4-Stream olur. PHASE-06A vendor-bağımsız `ci8` giriş, frame sözleşmesi ve frame-istatistik temelini bit-doğru model/testbench ile kurmuştur. PHASE-06B sabit nokta periyodik Hann bloğunu ve gelecekteki FFT'ye uygun açık AXI çıkışını bit-doğru Python/RTL eşdeğerliğiyle doğrular; FFT veya detector uygulamaz. Sonraki kontrollü adımlarda AMD/Xilinx FFT IP değerlendirilir ve ilk RTL tespit bloğu PHASE-03 `regional` detectorü olur; her PL bloğunun sabit nokta, gecikme, kaynak ve RTL sonucu ayrıca doğrulanır. Görsel profilin otomatik HDL ürettiği iddia edilmez. |
...
PHASE-06B sonrasındaki FFT IP entegrasyonu, FFT ölçekleme sözleşmesi, lineer güç ve PHASE-03 `regional` detector RTL adımlarının kesin faz kimlikleri ilgili planlama turunda ayrıca kurulacaktır. PHASE-06B bu kimlikleri veya uygulanmamış işlevleri mevcut saymaz.
